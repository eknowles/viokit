...
| TDR-002 | Console stack & client state | in-review 2026-08-23 | React + Vite; state in `effect/unstable/reactivity` atoms with a small in-repo `useAtom`; no routing framework; view state deliberately ephemeral until TDR-012 |
...
| TDR-008 | Schema→form generation — build vs adopt | in-review 2026-08-23 | build a small renderer over the Draft-2020-12 documents `catalog_describe` already publishes, raw-JSON fallback, custom components per transform; `@rjsf` deferred |
