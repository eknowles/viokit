...
| TDR-019 | Browser transport — `Bun.WebView` vs Playwright vs raw CDP | decided 2026-08-23 | `Bun.WebView` (Bun 1.4) Chrome backend; egress bound via `--proxy-server` in `argv`, identities isolated by per-view `dataStore` directory (spike contradicted the docs on both); WebKit excluded — no proxy control, would bypass egress; Bun 1.4 as a project-local devDependency, not a system upgrade |
